...
export default function Settings() {
    ...
    return (
        <div className="glass-panel" style={{ padding: '32px', flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '32px' }}>
            <div>
                <h2 style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '12px', marginBottom: '24px' }}>Appearance Settings</h2>
                        ...
                        {colors.map(c => (
                            <div
                                ...
                                style={{
                                    ...
                                    gap: '12px',
                                    cursor: 'pointer',
                                    border: themeColor === c.id ? `2px solid ${c.color}` : '1px solid var(--glass-border)',
                                    boxShadow: themeColor === c.id ? `0 0 15px ${c.color}40` : 'none',
                                    transition: 'all 0.2s ease'
                                }}
                                onClick={() => setThemeColor(c.id)}
                            >
                                <div style={{ width: '40px', height: '40px', borderRadius: '50%', backgroundColor: c.color, boxShadow: `0 0 10px ${c.color}` }}></div>
                                <span style={{ fontWeight: themeColor === c.id ? '600' : '400', color: themeColor === c.id ? 'var(--text-main)' : 'var(--text-muted)' }}>{c.label}</span>
                            </div>
                        ))}
                    </div>
                </div>
                        ...
                        {fonts.map(f => (
                            <div
                                ...
                                style={{
                                    padding: '16px 24px',
                                    cursor: 'pointer',
                                    border: fontFamily === f ? '2px solid var(--theme-accent)' : '1px solid var(--glass-border)',
                                    color: fontFamily === f ? 'var(--theme-accent)' : 'var(--text-muted)',
                                    fontWeight: fontFamily === f ? '600' : '400',
                                    fontFamily: f === 'Fira Code' ? "'Fira Code', monospace" : f === 'Roboto' ? "'Roboto', sans-serif" : "'Inter', sans-serif",
                                    transition: 'all 0.2s ease'
                                }}
