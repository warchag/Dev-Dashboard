...
export default function NetworkManager() {
    ...
    const [metrics, setMetrics] = useState<{ min: number; avg: number; max: number; loss: number } | null>(null)
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', height: '100%' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                            <input
                                ...
                                style={{ flex: 1, background: 'rgba(0,0,0,0.2)', border: '1px solid var(--glass-border)', padding: '8px 12px', borderRadius: '6px', color: 'white', outline: 'none' }}
                            />
                            <button className="btn" onClick={() => handlePing(customLocalIp)} disabled={loading} style={{ background: 'var(--theme-accent)', color: 'white' }}>
                                Ping
                            </button>
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                            <input
                                ...
                                style={{ flex: 1, background: 'rgba(0,0,0,0.2)', border: '1px solid var(--glass-border)', padding: '8px 12px', borderRadius: '6px', color: 'white', outline: 'none' }}
                            />
                            <button className="btn" onClick={() => handlePing(customExtIp)} disabled={loading} style={{ background: 'var(--theme-accent)', color: 'white' }}>
                                Ping
                            </button>
                        </div>
                    </div>
            ...
            {metrics && (
                <div style={{ display: 'flex', gap: '16px' }}>
                    <div className="glass-panel" style={{ padding: '16px', flex: 1, textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: '24px', fontWeight: 'bold', color: metrics.avg > 150 ? '#ff4757' : '#2ed573' }}>
                            {metrics.avg} ms
                        </div>
                    </div>
                    <div className="glass-panel" style={{ padding: '16px', flex: 1, textAlign: 'center' }}>
                        <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Packet Loss</div>
                        <div style={{ fontSize: '24px', fontWeight: 'bold', color: metrics.loss > 0 ? '#ff4757' : '#2ed573' }}>
                            {metrics.loss}%
                        </div>
                    </div>
                    <div className="glass-panel" style={{ padding: '16px', flex: 1, textAlign: 'center' }}>
                        <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Min / Max</div>
                        <div style={{ fontSize: '18px', fontWeight: '500', color: 'var(--text-main)', marginTop: '4px' }}>
            ...
            <div className="glass-panel" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    {loading && <span style={{ fontSize: '12px', color: 'var(--theme-accent)' }}>Running...</span>}
                </div>
                <pre
                    ref={terminalRef}
                    style={{
