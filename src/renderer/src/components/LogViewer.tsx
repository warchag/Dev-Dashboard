    ...
    return (
        <div className="glass-panel" style={{ padding: '24px', flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            >
                ...
                {!loading && !selectedContainer && <p style={{ color: 'var(--text-muted)' }}>Select a container to view logs.</p>}
                {!loading && selectedContainer && !logs && <p style={{ color: 'var(--text-muted)' }}>No logs available (or container is dead).</p>}
                {logs}
                <div ref={logsEndRef} />
            </div>
        </div>
