  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: '16px', flex: 1, minHeight: 0 }}>
        ...
        >
          {activeTool === 'json' && (
            <button className="btn" onClick={handleJsonFormat}>
              Format ➜
            </button>
          )}
          {activeTool === 'base64' && (
            <>
              <button className="btn" onClick={handleBase64E}>
                Encode ➜
              </button>
              <button className="btn" onClick={handleBase64D}>
                Decode ➜
              </button>
            </>
          )}
          {activeTool === 'url' && (
            <>
              <button className="btn" onClick={handleUrlE}>
                Encode ➜
              </button>
              <button className="btn" onClick={handleUrlD}>
                Decode ➜
              </button>
            </>
          )}
          {activeTool === 'jwt' && (
            <button className="btn" onClick={handleJwt}>
              Decode ➜
            </button>
          )}
